...
(function() {
  (function($, window) {
    return $(function() {
      var $md_editor, findEl, handleDragLeave, handleDragOver, handleDrop, retries, setupHandler;
      ...
      setupHandler = function() {
        ...
        return $md_editor.on('dragdrop drop', handleDrop);
      };
      handleDragOver = function(e) {
        e.preventDefault();
        e.stopPropagation();
        return $md_editor.css({
...
(function() {
  (function($, window) {
    return $(function() {
      var $aside, $content, $document, $window, aside_height, aside_offset, content_offset, margin, onScroll;
      $document = $(document);
      $window = $(window);
      $aside = $('.aside');
      $content = $('.aside-content');
      if (!($aside[0] && $content[0])) {
        return;
      }
      ...
      onScroll = _.debounce(function() {
        var bottom, content_height, document_top, left, position, top;
        aside_offset || (aside_offset = $aside.offset());
        content_offset || (content_offset = $content.offset());
...
(function() {
  (function($, window) {
    return $(function() {
      var $body, $close_btn, $document, $email, $email_input, $lightbox, $message, $nav, $newsletter, $parallax, $parallax_height, $parallax_offset, $quote, $quote_content, $quote_person, $quote_selector, $submit, $window, VIDEO_EMBED, closeOnEscape, closeVideo, dataType, emailErrorMessage, emailRegex, handleEmailSubmission, hasValue, initQuotes, invalidEmailMsg, lightbox_template, navHeight, onScroll, quote_rotate_current_index, quote_rotate_interval, quote_rotate_speed, quotes, rotateQuotes, scrollClass, selectQuote, setActiveBubble, showVideo, submitted, switchQuote, toggleQuote, url;
      $document = $(document);
      $nav = $('.doodles-nav');
      $body = $('.body');
      $window = $(window);
      ...
      closeVideo = function() {
        ...
        return $lightbox.addClass("is-closing");
      };
      showVideo = function() {
        var $close, $el, iframe_url, lightbox, project_title, video_id, video_service;
        $el = $(this);
        video_service = $el.data('video-service') || 'vimeo';
        video_id = $el.data('video-id');
        project_title = $el.data('project-title');
        iframe_url = VIDEO_EMBED[video_service]({
          video_id: video_id
        });
        lightbox = lightbox_template({
        ...
        return $lightbox.addClass("is-opened");
      };
      ...
      "content person".split(" ").map(function(selector) {
        return $quote.append("<div class='quote-" + selector + "'></div>");
      });
      ...
      initQuotes = function() {
        quotes.map(function(quote, index) {
          ...
          return $quote_selector.append($selector);
        });
        $quote_selector.on('click', '.quote-selector-bubble', function(e) {
          var $target, quote_id;
          $target = $(e.currentTarget);
          quote_id = $target.data('quote-id');
          selectQuote(quote_id);
          return quote_rotate_current_index = quote_id;
        });
        $('.quote').on('mouseenter', function() {
          return clearInterval(quote_rotate_interval);
        });
      ...
      rotateQuotes = function() {
        return quote_rotate_interval = setInterval(function() {
          var i;
          i = quote_rotate_current_index += 1;
          i = i % 3;
          return selectQuote(i);
        }, quote_rotate_speed);
      };
      setActiveBubble = function(active_index) {
        if (active_index == null) {
          active_index = 0;
        }
        if (!((0 <= active_index && active_index < quotes.length))) {
          return;
        }
        return $quote_selector.children().map(function(index, bubble) {
          var $bubble;
          $bubble = $(bubble);
          return $bubble.toggleClass("is-active", index === active_index);
        });
      };
      selectQuote = function(index) {
        ...
        return $quote.one("transitionend", function() {
          switchQuote(quote);
          return toggleQuote();
        });
      };
      toggleQuote = function(hide) {
        if (hide == null) {
          hide = !$quote.hasClass("is-hidden");
        }
        return $quote.toggleClass("is-hidden", hide);
      };
      switchQuote = function(quote) {
        if (!((quote != null) && (quote.person != null) && (quote.content != null))) {
          return;
        }
        $quote_content.text(quote.content);
        return $quote_person.text(quote.person);
      };
      initQuotes();
      hasValue = _.debounce(function(e) {
        var $this, has_value;
        $this = $(this);
        has_value = !!($this.val() || "").length;
        return $this.toggleClass("has-value", has_value);
      });
      ...
      handleEmailSubmission = function(e) {
        ...
        return request.fail(failedEmailSubmission);
      };
      ({
        successfulEmailSubmission: function() {
          $newsletter.addClass("is-success");
          $newsletter.removeClass("is-failed");
          return $newsletter.removeClass("is-loading");
        },
        failedEmailSubmission: function() {
          $newsletter.addClass("is-failed");
          return $newsletter.removeClass("is-loading");
        }
      });
      $submit.on('click', handleEmailSubmission);
      return $email_input.on('keyup keydown', _.debounce(function(e) {
        if (e.which === 13) {
          return handleEmailSubmission(e);
        } else if ($email_input.val().length === 0) {
          return $email.removeClass("is-error");
        }
      }));
    });
  })(window.$ || window.jQuery || window.Zepto, window);
...
(function() {
  (function($, window) {
    return $(function() {
      var appendInspirationImage, appendVimeoVideo, setupListeners;
      setupListeners = function() {};
      appendVimeoVideo = function() {
        var $el, $intro, $video, iframe_url, video_id;
        $video = $('.video-intro');
        if (!($video.length > 0)) {
          return;
        }
        ...
        $el = $("<blockquote>\n  <div class=\"frame frame--16x9\">\n    <iframe src=\"" + iframe_url + "\"\n      frameborder=\"0\"\n      webkitallowfullscreen\n      mozallowfullscreen\n      allowfullscreen>\n    </iframe>\n  </div>\n</blockquote>");
        return $intro.after($el);
      };
      appendInspirationImage = function() {
        var $el, $inspiration, $inspiration_image, description, title, url;
        $inspiration_image = $('.inspiration-image');
        if (!($inspiration_image.length > 0)) {
          return;
        }
        ...
        return $inspiration.after($el);
      };
      appendInspirationImage();
      return appendVimeoVideo();
    });
  })(window.$ || window.jQuery || window.Zepto, window);
...
(function() {
  (function($, window) {
    return $(function() {
      var $body, $members, $modal, $modal_close, $photo, $title, changeModalContent, showModal, toggleModal;
      $members = $('.member-grid');
      $modal = $('#modal-member-bio');
      if (!($members[0] && $modal[0])) {
        return;
      }
      ...
      changeModalContent = function(arg) {
        var body, photo, title;
        title = arg.title, body = arg.body, photo = arg.photo;
        $title.text(title);
        $body.empty();
        $body.append(body);
        $photo.empty();
        return $photo.append(photo);
      };
...
(function() {
  (function($, window) {
    return $(function() {
      var $academy_modal, level, openModal, project_slug, selectAcademy, selectProject, setupListeners, toggleAcademyModal, toggleModal;
      project_slug = null;
      level = null;
      ...
      setupListeners = function() {
        ...
        return $('.project-grid').on('click', '.project-grid-item', selectProject);
      };
      selectProject = function(e) {
        var $project;
        $project = $(e.currentTarget);
        project_slug = $project.data('project-slug');
        return toggleAcademyModal(true);
      };
      selectAcademy = function(e) {
        var $academy, location;
        ...
        return toggleAcademyModal();
      };
      toggleModal = function($modal, toggle) {
        if (toggle == null) {
          toggle = false;
        }
        $modal.toggleClass('is-active', toggle);
        return $modal.on('click.close-modal', '.modal-close', function(e) {
          toggleModal($modal);
          return $modal.off('.close-modal');
        });
      };
      toggleAcademyModal = function(toggle) {
        return toggleModal($academy_modal, toggle);
      };
      openModal = function(e) {
        var $modal, $modal_link, modal_id;
        $modal_link = $(e.currentTarget);
        modal_id = $modal_link.data('modal-id');
        $modal = $("#" + modal_id);
        return toggleModal($modal, true);
      };
